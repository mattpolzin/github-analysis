...
class CodeStatTests: XCTestCase {
	...
	func test_CodeStatsAddTogetherCorrectly() {
		property("The components of two CodeStats added together are equal to the components added separately.") <- forAll { (codeStat1: CodeStat) in
			return forAll { (codeStat2: CodeStat) in
	...
	func test_CodeStatMutatingAdditionEqualsNonMutatingAddition() {
		property("Mutating a code stat by adding another one to it is equivalent to adding two code stats together.") <- forAll { (codeStat1: CodeStat) in
			return forAll { (codeStat2: CodeStat) in
